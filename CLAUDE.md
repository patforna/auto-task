...
`skills/panel`, `skills/synthesize`, and `skills/tdd` are **vendored copies** from the `core-skills` plugin (`~/github/core-skills`), which is their source of truth. Do **not** edit them in this repo — edits here will be overwritten on the next re-vendor and silently diverge from core-skills.
