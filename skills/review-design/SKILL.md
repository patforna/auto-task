...
The serve command comes from the project bindings (`.claude/auto-task.md`, design-review section) — e.g. a recipe that serves test fixtures. If no binding exists, look for an obvious fixture-backed dev-server; if none, fail loudly — do not review against live/non-deterministic data.
