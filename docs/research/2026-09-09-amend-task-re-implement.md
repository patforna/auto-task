...
4. **One new status value, `amended`**, with a single backwards edge `ready-for-signoff | in-dev → amended → in-dev`. It costs zero config files: `settings.task_store.status` owns *how* status is written, not which values exist, so CLAUDE.md's four-file rule is not triggered.
...
- **impl-task Step 6** — the only step in the pipeline that waits for a human: "Wait for the user to provide feedback. Once provided, address it, commit, respond." Free-text and unbounded, but it writes no task file, moves no status, and triggers no re-review.
...
- **`[further user instructions]`** — present on every skill's Usage line, so an amendment can be *spoken* into any step. It is never persisted.
...
| plan-task Step 6 | "If the user instructed you to do so, write the Implementation Plan section to the task file." | Writes *the* section wholesale. No append, no supersede: re-planning destroys the record of what was built. |
| plan-task Step 2 | "For an epic sub-task, also read the epic file's `Locked Decisions` section and the `Implementation Notes` of already-done sibling sub-tasks." | Reading prior Implementation Notes is scoped to epic sub-tasks only, so re-planning an amended standalone task plans blind to its own first pass. |
...
| create-task § Task states | "`new` → `ready-for-dev` → `in-dev` → `ready-for-signoff` → `done`. Off-ramp states: `rejected` … `later`" | The sole status vocabulary source. Strictly monotone, no reopen edge, no state meaning "implemented, intent has since moved", both off-ramps terminal. |
| ship-task Step 1 | "Branch must be `task/NNN-...` with no uncommitted tracked changes." | No status precondition, so a task whose intent changed but was never re-implemented can still be shipped. |
...
- **impl-task Step 1** — gates on plan *presence* only, never on status, then writes `in-dev` unconditionally. This is the only backwards status write anywhere in the pipeline, and it is incidental rather than a designed reopen.
...
Semantics: *implemented, intent changed, a delta needs implementing.* Single new edge: `ready-for-signoff | in-dev → amended → in-dev`, then `ready-for-signoff` → `done` unchanged. Written only by `/at:amend-task`.
...
| impl-task Step 1 | Readiness accepts a plan carrying an amendment sub-section; the `in-dev` write is unchanged — already the correct forward move from `amended` |
| impl-task Step 3 | Scope clause: on an amended task, execute only the newest amendment block. Earlier steps are given state, still read in Step 2. This is the delta execution, and what keeps TDD honest |
...
| ship-task Step 1 | One clause: refuse on `status: amended`. The cheapest guard against shipping a half-amended task |
...
Per § 4, authored by `/at:amend-task` alone, which then re-gates the amended task via clarify-task. Apply create-task Step 4's decomposition triggers as a size fence: if the amendment runs to roughly ten or more ACs, or clusters into distinct themes, hand back to create-task instead of amending.

### Re-entry into auto-task: Step 3, status-driven, guard preserved

Steps 4–9 then run normally — the branch changed, so every review and gate must re-run. Step 2 is already satisfied by the delta plan, and Step 0's preflight still runs (cheap, and the roster may have moved). auto-task Step 1 gains one explicit "resuming an amended task" block rather than scattered conditionals:
...
**The guard is not weakened.** The refusal exists because "real work could be lost", and the amended re-entry is precisely the case that *wants* that work. The bypass keys on two conjoined facts — `status: amended` **and** a branch matching this task's number and slug — a combination only `/at:amend-task` produces. Every other stale-branch case still refuses.

Invocation: `/at:amend-task <task-path> [what changed]`, ending in a hand-off; `/at:auto-task NNN` then picks up `amended` and resumes. Neither panelist wanted an in-run interrupt hook.
...
9. **Track the amendment outside the task file** — every skill's Context section rests on "the task file plus repo state carry everything the next agent needs"; a second artefact breaks that invariant.
