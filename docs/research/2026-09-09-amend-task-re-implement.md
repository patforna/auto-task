...
| plan-task Step 6 | "If the user instructed you to do so, write the Implementation Plan section to the task file." | Writes *the* section wholesale. No append, no supersede: re-planning destroys the record of what was built. Step 6 itself is opt-in; in an auto-task run the write is Step 2's own last line ("Unless there are major flags, write the plan to the task file"), which consults neither status nor an existing plan — so any re-entry reaching Step 2 gets the wholesale write. |
...
**The signal is amendment-granular; the interruption it must survive is commit-granular.** Step 5 runs at the *end* of a run, after Step 3 has already committed work, so a run interrupted between the two leaves an `### Amendment N` block with no notes block — and Step 3's scope clause then re-selects the whole amendment, including steps whose code is already committed. That is § 1's impl-task Step 3 hard block verbatim, reached on a resume. No extra marker closes it: one written before the work lies if the run dies, one written after has the identical hole. What reconciles a mid-amendment resume is the branch — impl-task Step 2 already reads the code before Step 3 runs, and `A<N>.k` step ids let a resume match plan steps against landed commits and start at the first with none. That is a resume *procedure*, not a guard; how much of it impl-task should spell out is an open question for the follow-up task (§ Panel Limitations).

| Site | Change |
| --- | --- |
| create-task § Task states (vocabulary source) | Add `amended` and the one backwards edge |
| clarify-task Step 6 | One conditional — bump to `ready-for-dev` from the forward states, leave `amended` as `amended`. Without it auto-task Step 0.3 silently erases the declared intent and plan-task Step 1 then mis-gates; the guards are unaffected, since they read plan shape |
...
| impl-task Step 1 | Readiness accepts a plan carrying an amendment sub-section; the `in-dev` write is unchanged — the correct forward move from `amended` |
...
**What `amended` earns, now the guards read plan shape.** Not the operative test. Three narrower jobs: (i) plan-task Step 1's gate discriminator — the one site that must branch on status ("If the task's status is not `ready-for-dev` … flag it to the user"); (ii) the declared intent a human, or any scan over the task store's frontmatter, reads; (iii) ship-task's cheap human-facing signal beside the plan-shape refusal. **Cost of the obvious hack** (reuse `ready-for-dev`): plan-task Step 1 takes it as clearance to write *the* plan from scratch against a half-built repo, and every reader or scan sees a task queued for a first build that was in fact already built.
...
- **Left open by this doc:** how much of the mid-amendment resume procedure impl-task should spell out (§ 5). Plan shape marks an amendment as unimplemented, but not *how far* an interrupted run got; matching `A<N>.k` steps against landed commits is the reconciliation, and no marker closes the gap. All three are open questions for the follow-up task.
